...
%   comp.selection_metric_nfw : metric name used for NFW selection
  ...
  % -----------------------------------------------------------------------
  p = inputParser;
  addParameter(p, 'mRangeSol',             [2.0, 3.5],  @(x) isnumeric(x) && numel(x) == 2);
  addParameter(p, 'mRangeNFW',             [3.5, 10.0], @(x) isnumeric(x) && numel(x) == 2);
  addParameter(p, 'SelectionMetric',       'chi2',      @(x) ischar(x) || isstring(x));
  addParameter(p, 'RetryOnNoIntersection', false,       @(x) islogical(x) || isnumeric(x));
  addParameter(p, 'RetryTarget',           'soliton',   @(x) ischar(x) || isstring(x));
  addParameter(p, 'RetryField',            'min',       @(x) ischar(x) || isstring(x));
  addParameter(p, 'RetryStep',             0.2,         @(x) isnumeric(x) && isscalar(x) && x > 0);
  addParameter(p, 'RetryMax',              10,          @(x) isnumeric(x) && isscalar(x) && x >= 0);
  addParameter(p, 'Verbose',               true,        @(x) islogical(x) || isnumeric(x));
  addParameter(p, 'VerboseFindBestM',      true,        @(x) islogical(x) || isnumeric(x));
  parse(p, varargin{:});
  opt = p.Results;

  mRangeSol          = opt.mRangeSol;
  mRangeNFW          = opt.mRangeNFW;
  selectionMetricSol = lower(string(opt.SelectionMetric));
  selectionMetricNFW = lower(string(opt.SelectionMetric));
